...
fn main(@builtin(workgroup_id) workgroup_id: vec3<u32>, @builtin(num_workgroups) num_workgroups: vec3<u32>, @builtin(local_invocation_index) local_invocation_index: u32) {
    ...
    // // 下面的写法不对，会导致处理结果底噪变高，区别在于整数除法的精度损失（截断）顺序
    // let batch_idx = index / fft_len * 2u; // index / 8
    // let batch_offset = batch_idx * fft_len; // batch_idx * 16
    // let local_idx = index % (fft_len / 2u); // index % 8
...
fn bit_reversal_and_butterfly(idx: u32, n: u32, offset: u32) {
    ...
    let twiddle = vec2<f16>(1.0h, 0.0h); // 第一阶段的旋转因子是1
    // 计算输出位置 - 按正常顺序写入
...
fn fft_butterfly(idx: u32, n: u32, offset: u32, stage: u32) {
   ...
   //let a= vec2<f16>(1.0, 0.0);
   //let b= vec2<f16>(2.0, 0.0);
   // let b_twiddle = optimized_complex_mul(b, twiddle);
    let b_twiddle = complex_mul(b, twiddle);
    // 写回结果到同一缓冲区
