...
@group(0) @binding(0)
var<storage, read_write> buffer_a: array<vec2<f16>>;
@group(0) @binding(1)
var<storage, read_write> buffer_b: array<vec2<f16>>;
@group(0) @binding(2)
var<storage, read> twiddles: array<vec2<f16>>;

const PI: f16 = 3.14159265358979323846h;
...
fn main(@builtin(workgroup_id) workgroup_id: vec3<u32>, @builtin(num_workgroups) num_workgroups: vec3<u32>, @builtin(local_invocation_index) local_invocation_index: u32) {
    ...
    // 确定最终阶段
    let total_stages = u32(log2(f32(fft_len)) + 0.5);
    let final_stage = total_stages - 1u;
    ...
    if (stage == 0u) {
        ...
        if (local_idx < fft_len / 2u) {
            ifft_butterfly(local_idx, fft_len, batch_offset, stage + 1u, is_final_stage);
        }
    }
}
...
fn inverse_bit_reversal_and_butterfly(idx: u32, n: u32, offset: u32) {
    ...
    // 对于IFFT，第一阶段的旋转因子仍然是1
    let twiddle = vec2<f16>(1.0h, 0.0h);
    ...
    // 执行蝶形运算并写入 buffer_b
    // 对于IFFT，蝶形运算是相同的，因为第一阶段的twiddle factor是实数1
    let b_twiddle = complex_mul(b, twiddle);

    var result_a = a + b_twiddle;
...
// 标准 Cooley-Tukey 蝶形运算 - 只在 buffer_b 中操作 (IFFT版本)
fn ifft_butterfly(idx: u32, n: u32, offset: u32, stage: u32, is_final_stage: bool) {
    // 计算当前阶段的蝶形运算参数
    ...
    // 对twiddle factor取复数共轭
    let twiddle_raw = twiddles[twiddle_idx];
    let twiddle = vec2<f16>(twiddle_raw.x, -twiddle_raw.y);
    ...
    if (is_final_stage) {
        let scale = 1.0 / f32(n);
        result_a = vec2<f16>(f16(f32(result_a.x) * scale), f16(f32(result_a.y) * scale));
        result_b = vec2<f16>(f16(f32(result_b.x) * scale), f16(f32(result_b.y) * scale));
    }
...
fn complex_mul(a: vec2<f16>, b: vec2<f16>) -> vec2<f16> {
    return vec2<f16>(a.x * b.x - a.y * b.y, a.x * b.y + a.y * b.x);
}
