...
// Combine bit reversal and first stage radix-5 butterfly
fn radix5_bit_reversal_and_butterfly(idx: u32, n: u32, offset: u32) {
    ...
    // Apply radix-5 butterfly formula:
    ...
    // For x1 = t8 + it10, multiply t10 by i (swap components and negate real)
    let x1 = vec2<f32>(t8.x - t10.y, t8.y + t10.x);
    
    // For x2 = t9 + it11, multiply t11 by i
    let x2 = vec2<f32>(t9.x - t11.y, t9.y + t11.x);
    
    // For x3 = t9 - it11, multiply t11 by -i
    let x3 = vec2<f32>(t9.x + t11.y, t9.y - t11.x);
    
    // For x4 = t8 - it10, multiply t10 by -i
    let x4 = vec2<f32>(t8.x + t10.y, t8.y - t10.x);
...
// Standard radix-5 butterfly operation for stages > 0
fn radix5_butterfly(idx: u32, n: u32, offset: u32, stage: u32) {
    ...
    // Apply radix-5 butterfly formula with twiddle factors applied
    // Initial calculations
    let t1 = z1 + z4;
    let t2 = z2 + z3;
    let t3 = z1 - z4;
