...
fn main(@builtin(workgroup_id) workgroup_id: vec3<u32>, 
    ...
    if (stage == 0u) {
        ...
        if (local_idx < threads_per_fft) {
            radix3_bit_reversal_and_butterfly(local_idx, fft_len, batch_offset);
        }
    } else {
        // Stage 1 and above: Standard radix-3 butterfly
        if (local_idx < threads_per_fft) {
            radix3_butterfly(local_idx, fft_len, batch_offset, stage);
        }
    }
}

// Ternary bit reversal function
...
// Combine bit reversal and first stage radix-3 butterfly
fn radix3_bit_reversal_and_butterfly(idx: u32, n: u32, offset: u32) {
    // Compute log base 3 of n (rounded down)
    ...
    // Note: sin(π/3) = √3/2 = 0.866025404
    // For complex numbers, i * (a + bi) = -b + ai
    let t3 = vec2<f32>(
...
// Standard radix-3 butterfly operation for stages > 0
fn radix3_butterfly(idx: u32, n: u32, offset: u32, stage: u32) {
    // Calculate current stage butterfly parameters
    // For radix-3, we have 3^stage
    let m = pow3(stage);
    let step = 3u * m;
    let k = idx % m;
    let block_idx = idx / m;
