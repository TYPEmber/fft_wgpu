...
const MAX_SIZE: u32 = 4096u;
struct PushConstants { fft_len: u32, stage: u32 }
var<push_constant> consts: PushConstants;
var<workgroup> shared_in: array<vec2<f32>, MAX_SIZE>;
var<workgroup> shared_out: array<vec2<f32>, MAX_SIZE>;
...
fn main(
    ...
    // 1. 初始化共享内存（带边界保护）
    for (var i = tid; i < MAX_SIZE; i += workgroup_size) {
        if i<n{
            shared_in[i] = input[i];}
        else{
            shared_in[i] = vec2<f32>(0.0);
        }
        shared_out[i] = vec2<f32>(0.0);
    }
    workgroupBarrier();
    ...
    for (var m = 1u; m < n; m *= 2u) {
        ...
        let active_threads = min(workgroup_size, total_pairs);   // 结合对数和工作组大小来确定活跃线程数
        ...
        for (var p = 0u; p < pairs_per_thread; p++) {
           ...
           // let base = s * block_size;
            ...
            let pos1_in = var1 + j; // 对应 Python 中的 x[var + j]
            let pos2_in = var1 + j + (n / 2u); // 对应 Python 中的 x[var + j + n/2]
            //==== 关键修改2：边界检查优化 ====
          ...
          // let theta = -PI * f32(j) / f32(J); // Stockham专用公式
           ...
           // 蝶形运算
            let a = shared_in[pos1_in];
            //let b = complex_mul(shared_in[pos2_in], twiddle);
            let b=shared_in[pos2_in];
           // ==== 关键修改4：输出位置计算 ====
           // let output_base = s * J;
            //let pos1_out = output_base + j;
            //let pos2_out = output_base + j + (n / 2u);
            let output_base = s * block_size;
            let pos1_out = output_base + j;
            let pos2_out = output_base + j + J;


            //带保护的写入
            if (pos1_out < MAX_SIZE) {
                shared_out[pos1_out] = a + b;
            }
            if (pos2_out < MAX_SIZE) {
                shared_out[pos2_out] = complex_mul((a - b),twiddle);
            }
        }
    ...
    // 3. 输出结果（带保护）
    for (var i = tid; i < n; i += workgroup_size) {
        output[i] = shared_in[i];
        output[i]=input[i];
    }
}
